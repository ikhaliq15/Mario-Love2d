...
function TiledMap_GetSolid()
  local xml = LoadXML(love.filesystem.read("map/map01.tmx"))
  for k, sub in ipairs(xml) do
    if sub.label == "map" then
      for k, sub in ipairs(sub) do
        if (sub.label == "layer" and sub.xarg.name == "solid") then --  and sub.xarg.name == layer_name
          -- print(sub.xarg.name)
          -- print(sub[1][1].xarg.value)
          return sub[2][1]
        end
      end
    end
